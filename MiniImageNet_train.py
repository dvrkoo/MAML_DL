...
def main():
    ...
    for epoch in range(args.epoch // 10000):
        print("Epoch:", epoch, "of", args.epoch // 10000)
        # fetch meta_batchsz num of episode each time
        db = DataLoader(
            mini, args.task_num, shuffle=True, num_workers=1, pin_memory=True
        )

        for step, (x_spt, y_spt, x_qry, y_qry) in tqdm(
            enumerate(db),
            total=len(db),
            ...
            if step % 500 == 0:  # evaluation
                ...
                tqdm.write(f"Test acc: {accs}")  # Use tqdm.write() for clean output
